...
| Phase | Source code | Results / checkpoints | Deliverables | Dashboard |
|---|---|---|---|---|
| **Phase 1** — Foundational ML baselines (XGBoost, LSTM daily) | [models/baseline_xgboost.py](models/baseline_xgboost.py), [models/baseline_lstm.py](models/baseline_lstm.py), [models/hmm_train_2011.py](models/hmm_train_2011.py) | [results/backtesting/phase1_baselines/](results/backtesting/phase1_baselines/), [checkpoints/xgboost.joblib](checkpoints/xgboost.joblib), [checkpoints/best.pt](checkpoints/best.pt) | [report/phase1/Phase1_PPT.pdf](report/phase1/Phase1_PPT.pdf) | Phase 1 — Foundational ML |
| **Phase 2** — RAMT custom transformer (failed) | [models/ramt/](models/ramt/), [models/ablation_engine.py](models/ablation_engine.py), [models/inspect_attention.py](models/inspect_attention.py), [scripts/regenerate_ramt_outputs.py](scripts/regenerate_ramt_outputs.py) | [results/models/ramt/](results/models/ramt/) (metrics, predictions, training history, 8 `.pt` checkpoints, attention) | [report/phase2/Phase2_Report.pdf](report/phase2/Phase2_Report.pdf) | Phase 2 — Deep Learning |
| **Phase 3** — Foundation model + LoRA + hybrid | [models/lora_experiment/](models/lora_experiment/) (incl. trained adapter `.pt`), [scripts/train_chronos_lora.py](scripts/train_chronos_lora.py), [scripts/run_diagnostic_ablation.py](scripts/run_diagnostic_ablation.py), [scripts/run_hybrid_minus_momentum.py](scripts/run_hybrid_minus_momentum.py), [backtest/core/momentum_hmm_strategy.py](backtest/core/momentum_hmm_strategy.py) | [results/models/lora/](results/models/lora/) (metrics, predictions, `chronos_lora_adapter.pt`), [results/backtesting/{final_strategy,hybrid_lora,hmm_ablation,historical_2012}/](results/backtesting/), [results/ablation_summary.json](results/ablation_summary.json) | [report/phase3/Phase3_PPT.pdf](report/phase3/Phase3_PPT.pdf), [report/phase3/Phase3_Report.pdf](report/phase3/Phase3_Report.pdf) | Phase 3 — Hybrid system |
| **Canonical write-up** (all phases) | — | — | [report/report.pdf](report/report.pdf) (IEEE LaTeX, 10 pages) | Reviewer overview |
| Shared infrastructure | [features/](features/), [backtest/core/backtest.py](backtest/core/backtest.py), [data/](data/), [dashboard/app.py](dashboard/app.py), [report/](report/), [docs/](docs/) | n/a | n/a | All sections |
