...
def main() -> None:
    ...
    metrics_payload = {
        "model": "baseline_lstm_walkforward",
    ...
    with open(RESULTS_DIR / "lstm_metrics.json", "w", encoding="utf-8") as f:
        json.dump(metrics_payload, f, indent=2)

    print()
    print(f"Saved predictions → {out_path.resolve()}")
    print(f"Saved metrics   → {(RESULTS_DIR / 'lstm_metrics.json').resolve()}")
    print()
