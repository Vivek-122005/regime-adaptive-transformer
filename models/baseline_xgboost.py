...
def load_ticker_frame(ticker: str, fname: str) -> pd.DataFrame:
    """Load CSV or Parquet (same stem) from ``data/processed``."""
    path = PROCESSED_DIR / fname
    if path.is_file():
        return pd.read_csv(path, parse_dates=["Date"])
    stem = Path(fname).stem
    pq = PROCESSED_DIR / f"{stem}.parquet"
    if pq.is_file():
        ...
        return df
    raise FileNotFoundError(f"No {path} or {pq}")


def resolve_feature_files() -> list[tuple[str, str]]:
    out: list[tuple[str, str]] = []
    for ticker, fname in FEATURE_FILES:
        try:
            load_ticker_frame(ticker, fname)
            out.append((ticker, fname))
        except FileNotFoundError:
            print(f"Skip missing data: {ticker} ({fname})")
    return out
...
def prepare_xy(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series, pd.Series, pd.Series]:
    ...
    if "Log_Return" not in df.columns:
        if "Daily_Return" in df.columns:
            df = df.copy()
            df["Log_Return"] = df["Daily_Return"].astype(float)
        else:
            raise ValueError("Need Log_Return or Daily_Return")
...
def main() -> None:
    ...
    files = resolve_feature_files()
    if not files:
        raise SystemExit("No ticker feature files found under data/processed (csv or parquet).")

    data_raw: dict[str, pd.DataFrame] = {}
    for ticker, fname in files:
        print(f"Loading from: {ticker}")
        data_raw[ticker] = load_ticker_frame(ticker, fname)
            ...
            all_rows.append(
                {
                    ...
                    "Ticker": ticker,
                    "predicted_alpha": float(yp),
                    "actual_alpha": float(yt),
                    "Period": "Test",
                }
            )
    ...
    def _mean_ic_cs(df: pd.DataFrame) -> float:
        ics: list[float] = []
    ...
    pred_a = out_df["predicted_alpha"].values
    act_a = out_df["actual_alpha"].values
    glob_rmse = float(np.sqrt(np.mean((pred_a - act_a) ** 2)))
    glob_mae = float(np.mean(np.abs(pred_a - act_a)))
    glob_da = float(np.mean(np.sign(pred_a) == np.sign(act_a)) * 100.0)
    metrics_payload = {
        "model": "baseline_xgboost_walkforward",
        "directional_accuracy": glob_da / 100.0,
        "DA_pct": glob_da,
        "mean_IC": _mean_ic_cs(out_df),
        "rmse": glob_rmse,
        "mae": glob_mae,
        "n_rows": int(len(out_df)),
        "predictions_csv": str(out_path.resolve()),
    }
    with open(RESULTS_DIR / "xgboost_metrics.json", "w", encoding="utf-8") as f:
        json.dump(metrics_payload, f, indent=2)

    print()
    print(f"Saved predictions → {out_path.resolve()}")
    print(f"Saved metrics   → {(RESULTS_DIR / 'xgboost_metrics.json').resolve()}")
    print()
